...
In average the person takes `r meansteps` steps per day and the median steps per day is `r mediansteps`.
...
The `r intervalwithmax`th interval contains the maximum number of  average steps that is taken across all the days in the dataset.
...
```
The total number of rows with missing values are `r totalNA`.
...
The mean and median total number of steps taken per day for filled data is `r meantotalsteps` and `r mediantotalsteps`.
...
The mean total number of steps is same as previous value `r meantotalsteps` while the median total steps is  increased by `r diff`. 
